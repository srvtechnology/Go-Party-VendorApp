...
Future<List<ServiceModel>> getServiceList(AuthProvider auth) async {
  Response response = await Dio().get(
      "${APIConfig.baseUrl}/api/service-list-vendor",
...
Future createService(
    AuthProvider auth, Map<String, dynamic> serviceData) async {
  try {
    Response response = await Dio().post(
      "${APIConfig.baseUrl}/api/service-insert",
      options: Options(headers: {"Authorization": "Bearer ${auth.token}"}),
...
Future<bool> deleteService(AuthProvider auth, String serviceId) async {
  try {
    Response response = await Dio().get(
      "${APIConfig.baseUrl}/api/service-delete/$serviceId",
      options: Options(headers: {"Authorization": "Bearer ${auth.token}"}),
    );
    return true;
  } catch (e) {
...
Future<bool> updateService(
  ...
  try {
    Response response = await Dio().post(
        "${APIConfig.baseUrl}/api/service-update",
        options: Options(headers: {"Authorization": "Bearer ${auth.token}"}),
...
Future<ServiceDropDownOptions> getServiceOptions(dynamic auth) async {
  try {
    ...
    Response response = await Dio().get(
      "${APIConfig.baseUrl}/api/service-list-add-view",
      options: Options(headers: {"Authorization": "Bearer ${auth.token}"}),
    );
