...
Future<String> login(String email, String password) async {
  ...
  try {
    response = await dio.post("${APIConfig.baseUrl}/api/vendor/login",
...
Future<bool> signUpMain(Map data) async {
  ...
  try {
    response = await Dio()
        .post("${APIConfig.baseUrl}/api/vendor/register", data: data);
    return true;
  } catch (e) {
    if (e is DioError) {
      CustomLogger.error(e.response?.data);
...
Future<String> resetPassword(String userId, String password, String otp) async {
  ...
  try {
    response = await dio.post("${APIConfig.baseUrl}/api/vendor/reset-password",
        data: FormData.fromMap(
            {"otp": otp, "user_id": userId, "password": password}));
...
Future<bool> completeRegistration(AuthProvider state, Map data) async {
  try {
    Response response = await Dio().post(
        "${APIConfig.baseUrl}/api/vedor-register-part-registration",
        data: data,
...
Future<bool> completeRegistration2(
    dynamic state, Map<String, dynamic> data) async {
  try {
    Response response = await Dio().post(
        "${APIConfig.baseUrl}/api/vendor-register-part-three-registration",
...
Future<bool> completeRegistrationIntermediate(
    AuthProvider state, Map<String, dynamic> data) async {
  try {
    Response response = await Dio().post(
        "${APIConfig.baseUrl}/api/vendor-register-part-two-registration",
        data: FormData.fromMap(data),
        options: Options(headers: {"Authorization": "Bearer ${state.token}"}));
    CustomLogger.debug(response.data);
    return true;
...
Future<bool> completeRegistration3(
    AuthProvider state, Map<String, dynamic> data) async {
  try {
    CustomLogger.debug(data);
    Response response = await Dio().post(
        "${APIConfig.baseUrl}/api/vendor-register-part-four-registration",
        data: FormData.fromMap(data),
        options: Options(headers: {"Authorization": "Bearer ${state.token}"}));
    CustomLogger.debug(response.data);
    return true;
...
Future completeRegistrationTerms(AuthProvider auth) async {
  try {
    Response res = await Dio().post(
        "${APIConfig.baseUrl}/api/update-terms-condition-register-status",
        data: {"vendor_reg_part": 7},
...
Future<void> deactivateAccount(AuthProvider auth) async {
  try {
    Response response = await Dio().get(
        "${APIConfig.baseUrl}/api/manage-vendor/inactive",
        options: Options(headers: {"Authorization": "Bearer ${auth.token}"}));
  } catch (e) {
    if (e is DioError) {
      CustomLogger.error(e.response?.data);
...
Future<void> activateAccount(AuthProvider auth) async {
  try {
    Response response = await Dio().get(
        "${APIConfig.baseUrl}/api/manage-vendor/active",
        options: Options(headers: {"Authorization": "Bearer ${auth.token}"}));
    CustomLogger.debug(response.data);
  } catch (e) {
    if (e is DioError) {
...
Future<void> deleteAccount(AuthProvider auth) async {
  try {
    Response response =
        await Dio().post("${APIConfig.baseUrl}/api/api/manage-vendor/delete");
  } catch (e) {
    if (e is DioError) {
      CustomLogger.error(e.response?.data);
    }
