...
class _HomepageState extends State<Homepage> {
  ...
  Widget successCard(
      String? address, String? shippingName, VoidCallback onClose) {
    print(">>>>$address");
    return Stack(
      children: [
        // Card Container
...
class _ProfileState extends State<Profile> {
  ...
  void initState() {
    ...
    selectedOfficeCountry =
        Provider.of<AuthProvider>(context, listen: false).user?.officeCountry ??
    ...
    return Consumer<AuthProvider>(builder: (context, auth, child) {
      return Container(
        ...
        child: DefaultTabController(
            ...
            child: Column(
              children: [
                _ProfileHeader(context, auth, auth.user!.name, auth.user!.email,
                    auth.user!.vendorUrl),
                SizedBox(
                  height: 10,
                ),
                Expanded(
                  ...
                  key: _profile,
                  description:
                      "This is the profile section. You can edit / view your details",
                  child: Container(
            ...
            Container(
              ...
              child: IconButton(
                ...
                icon: Icon(Icons.edit,
                    color:
                        OfficeEditMode ? Theme.of(context).primaryColor : null),
              ),
            ),
            _CustomText(context,
            ...
            if (OfficeEditMode)
              Column(
                children: [
                  ...
                  // Temporarily commented out CSCPicker due to compatibility issues
                  Container(
                    padding: EdgeInsets.symmetric(horizontal: 20, vertical: 10),
                    child: Text("Location picker temporarily disabled"),
                  ),
                  _CustomText(context,
                      editMode: OfficeEditMode,
                      title: "PinCode",
          ...
          children: [
            ...
            if (ProfileEditMode)
              ...
              _CustomText(context,
                  ...
                  content: kyctypes
                          .firstWhere(
                              (element) => element.value == auth.user!.kycType,
                              orElse: () => kyctypes[0])
                          .title ??
                      "",
                  canEdit: false),
          ...
          children: [
            Container(
              ...
              child: IconButton(
                ...
                icon: Icon(Icons.edit,
                    color: DocumentsEditMode
                        ? Theme.of(context).primaryColor
                        : null),
              ),
            ),
            _CustomImage(context,
                imageUrl: auth.user!.gstUrl ?? "", title: "GST"),
      ...
      onTap: () {
        if (imageUrl.endsWith("pdf")) {
          Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (context) => pdfViewer(pdfUrl: imageUrl)));
        } else {
          Navigator.push(
              context,
              MaterialPageRoute(
        ...
        child: Row(
          children: [
            if (imgPath[title] != null)
              Expanded(
                  child: Container(child: Image.file(File(imgPath[title]!))))
            else
              Expanded(
                child: Container(
                    child:
                        (imageUrl.isEmpty && imageUrl.endsWith("pdf") == false)
                            ? Text("")
                            : CachedNetworkImage(
                    ...
                    children: [
                      Expanded(
                        child: DropdownSearch<Country>(
                          ...
                          items: DefaultCountries.map((e) => Country(
                              id: e["id"].toString(),
                              name: e["name"])).toList(),
                        ),
                      ),
              ...
              children: [
                Container(
                    ...
                    child: editMode
                        ? TextFormField(
                            keyboardType:
                                validatePhone || accountConfirm || isPin
                                    ? TextInputType.phone
                                    : TextInputType.text,
                            validator: (text) {
  ...
  Widget _ProfileHeader(BuildContext context, AuthProvider auth, String name,
...
class _OrdersState extends State<Orders> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
        (_) => ShowCaseWidget.of(context).startShowCase([_order]));
  }
