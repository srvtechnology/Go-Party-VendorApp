...
class _WalletPageState extends State<WalletPage> {
  ...
  bool isBankDetailExist=false;

  GlobalKey<FormState> _bankFormKey = GlobalKey<FormState>();
  bool BankEditMode = false ;

  void _withdraw(BuildContext context, double availableAmount,double withdrawAmount) {
                  ...
                  children: [
                    ElevatedButton(
                        onPressed: () async {
                          ...
                          double enteramount=double.parse(_withdrawAmount.text);
                          if(enteramount <= availableAmount){
                            if (_formKey.currentState!.validate()) {
                              try {
                                bool isWithdraw = await withdrawAmountFromWallet(
                                ...
                                if (isWithdraw) {
                                  ...
                                  context
                                      .read<WalletProvider>()
                                      .refreshData(context.read<AuthProvider>());
                                } else {
                                  if (context.mounted) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text(
                                    "The amount that you entered is maximum from your available amount in wallet."),
                              ),
                            );
                          }
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          ...
                          ElevatedButton(
                            onPressed: () {
                              if(state.walletData.isBankExist){
      ...
      child: Row(
        children: [
          Expanded(
            ...
            radius: 15,
            child: transaction.transactionStatus.contains("debit") ? Text("D"):Text("C"),
          )),
          Expanded(
              flex: 2,
              child: Center(
                ...
                children: [
                  ...
                  Text(
                    formatDate(transaction.transactionDate.toString()).substring(0, 10),
